...
void autocorrelation_FFT(double* data, double* C, double* S){
	...
	double* stru = malloc( L*L*sizeof(*stru)); //structure factor as a function of wavevector
	if(stru==0){
		printf("allocation of 'stru' failed\n");
		free(stru);
		exit(1);
	}
	double* corr = malloc( L*L*sizeof(*corr));  //correlation as a function of relative position vector
	if(corr==0){
