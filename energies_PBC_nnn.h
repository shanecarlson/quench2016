//Energies for the next nearest neighbor square lattice with periodic boundary conditions
...
void energy_contribution(int i, int j){
	...
	EC += bond(s[i][j], s[dec(i)][inc(j)]);
	EC += bond(s[i][j], s[inc(i)][dec(j)]);
}
...
void proposed_energy_contribution(int i, int j){
	...
	EC += bond(s_proposed, s[dec(i)][inc(j)]);
	EC += bond(s_proposed, s[inc(i)][dec(j)]);
}
